  ...
  TASK:
    Analyze the architectural floor plan and highlighted wall segments accompanied by polygon vertices, it's perimeter wall endpoints and OCR extracted transcription entries from the floor plan to determine the following features,
      ...
      WALL_EXTRACTION_INSTRUCTIONS:
        ...
        - If the numerical dimension entity represents the exterior length of the wall which includes the width / thickness of the orthogonal wall it is joined with, the interior length is typically found inside the polygon the wall belongs to as a room size annotation identified as interior shape (<horizontal length> x <vertical length>) of the polygon. Use one of the dimensions (<horizontal length> / <vertical length>) depending on the orientation of the wall.
        - If the target wall is attached to another wall in orthogonal orientation, refer the numerical dimension that represents its interior length to derive the length of the wall which excludes the width of the orthogonal wall.
