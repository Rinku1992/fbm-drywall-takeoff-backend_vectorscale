  ...
  TASK:
    Analyze the architectural floor plan and highlighted wall segments accompanied by polygon vertices, it's perimeter wall endpoints and OCR extracted transcription entries from the floor plan to determine the following features,
      ...
      CEILING_EXTRACTION_INSTRUCTIONS:
        ...
        - The `tilt_axis` of a sloped ceiling is in the direction against the inclination. The `ceiling_axis` runs through the central axial line of the ceiling in the direction of the inclination. The `tile_axis` is one of the axial lines (x-> horizontal, y-> vertical). Can only have a value "horizontal" or "vertical" or "NULL". Mention "NULL" only if slope angle is 0. The slope of the `tile_axis` (angle of the ceiling) is measured against its axial line (x-> horizontal, y-> vertical).
